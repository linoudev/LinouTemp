...
#include "nt.hpp"
// util that is detected
std::wstring mod::utl::GetFullTempPath() {
...
bool mod::utl::ReadFileToMemory(const std::wstring& file_path, std::vector<BYTE>* out_buffer) {
...
bool mod::utl::CreateFileFromMemory(const std::wstring& desired_file_path, const char* address, size_t size) {
...
uint64_t mod::utl::GetKernelModuleAddress(const std::string& module_name) {
	void* buffer = nullptr;
	DWORD buffer_size = 0;
...
BOOLEAN mod::utl::bDataCompare(const BYTE* pData, const BYTE* bMask, const char* szMask) {
...
uintptr_t mod::utl::FindPattern(uintptr_t dwAddress, uintptr_t dwLen, BYTE* bMask, const char* szMask) {
...
PVOID mod::utl::FindSection(const char* sectionName, uintptr_t modulePtr, PULONG size) {
...
std::wstring mod::utl::GetCurrentAppFolder() {
	wchar_t buffer[1024];
